...
program allocation !module to test allocation logic module of LPJmL-Fire
    ...
    real(r_8) :: ltor = 1. !leaf:root from Sitch et al., 2003 (table 3)
    ...
    bminc=(/6.15,6.,6.18,6.6,6.5,6.8,6.3,6.,6.8,6.84,5.25,5.,5.2,6.7,&
    &6.18,8.6,5.5,3.8,3.3,2./)
    ...
    do pls = 1,npls
        ...
        if (rminc_ind_min(pls) .gt. 0. .and. lminc_ind_min(pls) .gt. 0. .and. &
            ...
            & 'SAP_INC(gC/ind)', (sminc_ind(pls)/1.D3)
            ! print*, 'BMINC', bminc_ind(pls)
    ...
    ! function fr (lm,sm,hm,rm,inc,ltor,sla,wooddens,x) result (root)
    !     implicit none
    ...
    !     real(r_8) :: allom3 = 0.5
    !     real(r_8) :: latosa = 8.e3
    !     real(r_8) :: pi   =  3.14159265

    !     !arguments

    !     real(r_8), dimension (:), intent(in) :: sla    !specific leaf area
    !     real(r_8), intent(in) :: wooddens
    !     real(r_8), dimension (:), intent(in) :: ltor  !leaf mass to root mass ratio
    !     real(r_8), dimension (:), intent(in) :: lm     !individual leaf mass
    !     real(r_8), dimension (:), intent(in) :: sm     !individual sapwood mass
    !     real(r_8), dimension (:), intent(in) :: hm     !individual heartwood mass
    !     real(r_8), dimension (:), intent(in) :: rm     !individual root mass
    !     real(r_8), dimension (:), intent(in) :: inc    !individual biomass increment
    !     real(r_8), dimension (:), intent(in) :: x      !leafmass allocation amount as input
    !     real(r_8), dimension(:) :: root
    ...
    !     root = a3*((sm(:)+inc(:)-x(:)-((lm(:)+x(:))/ltor)+rm(:)+hm(:))/wooddens)/pi4- &
    !             &((sm(:)+inc(:)-x(:)-((lm(:)+x(:))/ltor)+rm(:))/((lm(:)+x(:))*sla(:)*wooddens/latosa))**a2
